...
const tileToPosition = (tileX: number, tileY: number, size: number) => {
  return new Vector2((tileX * size * 3) / 2, tileY * Math.sqrt(3) * size + (((tileX % 2) * Math.sqrt(3)) / 2) * size);
};
  ...
  return (
    <Canvas camera={{ position: [5, 10, 25] }}>
      ...
      <mesh>
        ...
        {map.map((item, index) =>
          item.map((cell, nextIndex) => <ThreeHexItem cell={cell} position={tileToPosition(index, nextIndex, 1)} />),
        )}
      </mesh>
      <mesh geometry={mapS}>
        <meshStandardMaterial color={'#6c6c6c'}></meshStandardMaterial>
      </mesh>
      <OrbitControls target={[5, 0, 5]} enableDamping={true} dampingFactor={0.05} />
    </Canvas>
  );
}
